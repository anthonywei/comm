...
const uint32_t PKG_HEAD_LEN = sizeof(__STX__) + sizeof(uint32_t);
//版本信息
                           ...
                           int iFlag)
{
    ...
    // Keep this piece
	m_oByteStreamNet.resetStreamBuf(m_achRecvBuf + m_nRecvPos, m_dwMaxRecvBufLen - m_nRecvPos);
	m_oByteStreamNet.Write(pRecvBuf, nBufLen);
    ...
    {
		m_oByteStreamNet.resetStreamBuf(m_achRecvBuf + m_l_nPos, m_dwMaxRecvBufLen - m_l_nPos);
		m_oByteStreamNet >> wSTX;
		...
		m_oByteStreamNet >> m_l_nPacketLen;

        m_l_nPos += sizeof(m_l_nPacketLen);
        m_l_nCurrBufRecvSize -= sizeof(m_l_nPacketLen);
                             ...
                             )
{
	...
	if(nBufLen > 0) 
	{
		if((m_nSendPos + nBufLen + PKG_HEAD_LEN) > m_dwMaxSendBufLen)
		{
			m_bNeedOnSend = true;
    ...
    {
        if((uint32_t)szByteSent != m_l_nCurrBufSendSize)
        {
            //printf("NeedOnSend, szByteSent[%d] != m_l_nCurrBufSendSize[%d]\n", szByteSent, m_l_nCurrBufSendSize);
            assert(m_l_nCurrBufSendSize > (uint32_t)szByteSent);
